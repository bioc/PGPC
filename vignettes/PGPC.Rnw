...
\usepackage{calc}
%\usepackage{subfig} use subcaption instead, no problems with line breaks
...
\bioctitle[PGCP, 2014]{\textsf{\textbf{Systems pharmacology by integrating 
image-based phenotypic profiling and chemical-genetic interaction mapping}\\
